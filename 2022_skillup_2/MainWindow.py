...
class MainWindow(QMainWindow, DBManager):
    ...
    def setupUi(self):
        ...
        testList_groupbox_minimumWidth = (left_right_imgBtn_width * 2 + img_scrollArea_width) / 9 * 4       # 하단 평가목록 넓이
        all_groupbox_minimunWidth = (left_right_imgBtn_width * 2 + img_scrollArea_width) / 9                # 하단 ALL 버튼 넓이
        version_groupbox_minimunWidth = (left_right_imgBtn_width * 2 + img_scrollArea_width) / 9 * 3        # 하단 버전 정보 넓이
        result_groupbox_minimunWidth = (left_right_imgBtn_width * 2 + img_scrollArea_width) / 9             # 하단 진행 상황 넓이

        bottom_groupbox_fixedHeight = main_height / 2.4
        self.version_groupbox_minimumWidth = main_width / 2

        self.setMinimumSize(main_width, main_height)
        self.setGeometry(main_left, main_top, main_width, main_height)
        print(main_left, main_top, main_width, main_height, screen.width(), screen.height())
        ...
        img_VBoxLayout = QVBoxLayout(img_scrollAreaWidgetContents)
        img_VBoxLayout.setAlignment(Qt.AlignTop)
        ...
        testList_groupbox.setMinimumWidth(testList_groupbox_minimumWidth)
        testList_groupbox.setFixedHeight(bottom_groupbox_fixedHeight)
        self.testList_hboxLayout = QHBoxLayout()
        testList_groupbox.setLayout(self.testList_hboxLayout)

        self.set_testList_hboxLayout()

        self.bottom_hboxLayout.addWidget(testList_groupbox)

        # ALL PASS, ALL FAIL, ALL N/T, ALL N/A
        all_groupbox = QGroupBox("ALL")
        all_groupbox.setMinimumWidth(all_groupbox_minimunWidth)
        ...
        testAll_VBoxLayout.addSpacing(5)
        ...
        self.bottom_hboxLayout.addWidget(all_groupbox)

        # 버전 정보
        version_groupbox = QGroupBox("버전 정보")
        version_groupbox.setMinimumWidth(version_groupbox_minimunWidth)
        ...
        self.bottom_hboxLayout.addWidget(version_groupbox)

        # 진행 상황
        result_groupbox = QGroupBox("진행 상황")
        result_groupbox.setMinimumWidth(result_groupbox_minimunWidth)
        ...
        self.bottom_hboxLayout.addWidget(result_groupbox)

        right_VBoxLayout.addLayout(self.bottom_hboxLayout)
...
#         return result
    ...
    def set_testList_hboxLayout(self):
        ...
        self.testList, _ = self.sp.read_ini_test(table = "Test_List")

        try:
            for i in range(self.testList_hboxLayout.count()):
                self.testList_hboxLayout.itemAt(i).widget().deleteLater()
        ...
        if len(self.testList) != 0:
            ...
            for i,val in enumerate(self.testList):
                ...
                radio_buttons = [globals()[f'gb{i}_pass'], globals()[f'gb{i}_fail'], globals()[f'gb{i}_nt'], globals()[f'gb{i}_na'], globals()[f'gb{i}_nl']]
                ...
                for radio_button in radio_buttons:
                    testList_vboxLayout.addWidget(radio_button)

                globals()[f'testList_groupbox_{i}'].setLayout(testList_vboxLayout)

                self.testList_hboxLayout.addWidget(globals()[f'testList_groupbox_{i}'])
                self.testList_hboxLayout.setAlignment(Qt.AlignLeft)
...
#                 horizontalLayout.addLayout(self.img_VBoxLayout)

    @AutomationFunctionDecorator
    def set_field_gridLayout(self):
        self.fieldList, _ = self.sp.read_ini_test(table = "Field")
